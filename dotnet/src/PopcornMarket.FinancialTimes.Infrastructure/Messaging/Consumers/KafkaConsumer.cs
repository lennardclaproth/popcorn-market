        ...
        Task.Run(async () =>
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (message != null)
                    {
                        await handler.HandleAsync((dynamic) message, cancellationToken);
                        _logger.LogInformation("Handled event on topic: {Topic} as {Group}", result.Topic, _configuration.GetSection("Messaging:Kafka:GroupId"));
                    }
                }
                catch (OperationCanceledException) { }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Error while consuming messages. {Exception}", ex.Message);
                }
            }
        }, cancellationToken);
